...
def remove_chinese(text):
    """Remove Chinese characters and any text within parentheses containing Chinese characters"""
    # Remove Chinese characters (including punctuation and parentheses content with Chinese)
...
def extract_qa_from_docx(docx_path, questions_output, answers_output):
    """
    Extract questions and answers from a Tibetan DOCX file, re-index them, remove Chinese, and save to separate text files.
    """  # noqa
    try:
        # Read the DOCX file
        ...
        # Remove any Chinese characters from the full text
        full_text = remove_chinese(full_text)

        # Debug: Print first part of the cleaned text
        print("\nDebug: First 200 characters of cleaned text:")
        print(full_text[:200])

        # Pattern to match the full Q&A pairs based on 'དྲི་བ།' for questions and 'དྲིས་ལན།' for answers
        qa_pattern = r"དྲི་བ།\s*(.*?)\s*དྲིས་ལན།\s*(.*?)(?=དྲི་བ།|$)"
...
def main():
    ...
    # Configure paths
    input_file = current_dir / "data" / "input" / "་ཨེ་འགོག་ཉིན་རེའི་ལན་རེ།.docx"
