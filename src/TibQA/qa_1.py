...
def extract_qa_from_docx(docx_path, questions_output, answers_output):
    ...
    try:
        ...
        # Modify the regex pattern to ensure "དྲི་བ།" is a standalone word and not part of a larger word
        qa_pattern = r"\bདྲི་བ།\b\s*(.*?)\s*ལན།\s*(.*?)(?=\bདྲི་བ།\b|$)"
...
def main():
    ...
    # Configure paths
    input_file = current_dir / "data" / "input" / "རྒྱལ་རབས་དྲིས་ལན་བརྒྱ་པ། .docx"
